...
class RkaSSsatuController extends Controller {
    public function index(){
        return view('content.laporan.KLASIFIKASI.SASARAN.SATU.satu');
    }
    public function pdfNonApbn(){
        $idunit = session()->get("unitkerja", "");
        $tahun = str_replace("_", " ", session()->get("tahun", "tahun_2025"));
        return view('content.laporan.KLASIFIKASI.SASARAN.SATU.pdf', compact('idunit', 'tahun'));
    }
    public function getNonApbn() {
        $tahun = session()->get("tahun", "tahun_2025");
        $kro = DB::connection('sirekat')->select("SELECT DISTINCT kd_ss, sasaran_program FROM iku_baru WHERE kd_ss = 'SKL' ");
        $ro = DB::connection('sirekat')->select("SELECT DISTINCT SUBSTRING(kode_ikk,5) AS kode_ikk, indikator_kinerja_kegiatan FROM iku_baru
                            WHERE kode_ikk LIKE '%IKU.1%' ORDER BY kode_ikk DESC");
        $kp = DB::connection('sirekat')->select("SELECT DISTINCT kd_ikv, ikv FROM iku_baru WHERE kode_ikk LIKE '%IKU.1%' ORDER BY RIGHT(kd_ikv, 5) DESC");
        $sk = IKK::SELECT(\DB::raw("SUBSTRING(kd_keg, 4, 5) AS kd_keg_compare"),
                        \DB::raw("SUBSTRING(kd_keg, 10) AS kd_keg"), "rincian_kegiatan")->where('kode_ikk', 'like', '%IKU.1%')
                        ->distinct()->orderBy("kd_keg", "DESC")->get();
        // ✅ Select data unit kerja dari tabel rekat
        $unit_kerja = DB::connection('sirekat')->select("select DISTINCT tb_rekat.id,tb_rekat.kd_rk, tb_rekat.id_sub_judul,tb_rekat.unit_pelaksana, tb_rekat.rincian_komponen,
        tb_unit.idunit, tb_unit.unitkerja,tb_rekat.kd_rk from tb_rekat 
        inner join tb_unit ON tb_rekat.unit_kerja = tb_unit.idunit 
        WHERE tb_rekat.tahun = '$tahun' AND tb_rekat.sd = '41' 
        AND tb_rekat.kd_kro = 'SKL' GROUP BY tb_rekat.unit_pelaksana, tb_rekat.unit_kerja, tb_rekat.kd_rk");
        // ✅ Select data detail kegiatan dari tabel rekat
        $detail_kegiatan =  Rekat::SELECT("id","id_sub_judul", "unit_kerja","kd_rk", "sub_judul", "unit_pelaksana", "rincian_komponen", "unit_kerja", "rpd")
                            ->where(['tahun' => $tahun, "sd" => '41'])->where("kd_kro", "=", "SKL")->get();
        ...
        AND tb_rekat.kd_kro = 'SKL' AND tb_rekat.tahun = '$tahun' AND tb_rekat.sd = '41' 
        ...
        tb_rekat.sub_judul, tb_rekat.rincian_komponen AS rk_rekat, tb_rabkegiatan.id_kegiatan as id_keg, tb_rabperalatan.id_kegiatan as id_per, tb_rabgedung.id_kegiatan as id_gdg,
        ...
        FORMAT((tb_rabgedung.DED_AWAL + tb_rabgedung.DED_REVIEW + tb_rabgedung.nilai_perencanaan
               ...
               from tb_rekat LEFT join tb_rabkegiatan ON tb_rekat.id = tb_rabkegiatan.id_rekat 
               ...
               WHERE (tb_rabgedung.id_jenis_belanja LIKE '%51%' OR 
                    ...
                    tb_rabperalatan.id_jenis_belanja LIKE '%51%') AND tb_rekat.kd_kro = 'SKL'
                    AND tb_rekat.sd = '41' 
        ...
        $sum_coa = DB::connection('sirekat')->select("select tb_rekat.id_sub_judul,tb_rekat.unit_kerja,tb_rekat.kd_rk,
               ...
               WHERE (tb_rabgedung.id_jenis_belanja LIKE '%51%'
               OR tb_rabkegiatan.id_jenis_belanja LIKE '%51%'
               OR tb_rabperalatan.id_jenis_belanja LIKE '%51%') AND tb_rekat.kd_kro = 'SKL' AND tb_rekat.tahun = '$tahun' 
               AND tb_rekat.sd = '41' GROUP BY tb_rabkegiatan.id_jenis_belanja, tb_rabkegiatan.id_jenis_belanja,tb_rabgedung.id_jenis_belanja, tb_rekat.id_sub_judul ");
               ...
               WHERE (tb_rabgedung.id_jenis_belanja LIKE '%51%'
               OR tb_rabkegiatan.id_jenis_belanja LIKE '%51%'
               OR tb_rabperalatan.id_jenis_belanja LIKE '%51%') AND tb_rekat.kd_kro = 'SKL'
               AND tb_rekat.tahun = '$tahun'
               AND tb_rekat.sd = '41' GROUP BY tb_rekat.unit_kerja,tb_rekat.sub_judul");
               ...
               OR tb_rabkegiatan.id_jenis_belanja LIKE '%51%'
               OR tb_rabperalatan.id_jenis_belanja LIKE '%51%') AND tb_rekat.kd_kro = 'SKL' AND tb_rekat.tahun = '$tahun'
               AND tb_rekat.sd = '41' GROUP BY  tb_rekat.rincian_komponen, tb_rekat.unit_kerja, tb_rekat.sub_judul");
        ...
        $sum_ikv = DB::connection('sirekat')->select("SELECT 
            ...
            SUM(coalesce(tb_rabperalatan.jumlah_biaya,0)),0) as TOTAL_IKV
        FROM tb_rekat
        LEFT JOIN tb_rabkegiatan ON tb_rekat.id = tb_rabkegiatan.id_rekat
        LEFT JOIN tb_rabgedung ON tb_rekat.id = tb_rabgedung.id_rekat
        LEFT JOIN tb_rabperalatan ON tb_rekat.id = tb_rabperalatan.id_rekat
        ...
        GROUP BY tb_rekat.kd_keg");
               ...
               OR tb_rabkegiatan.id_jenis_belanja LIKE '%51%'
               OR tb_rabperalatan.id_jenis_belanja LIKE '%51%') AND tb_rekat.tahun = '$tahun' 
               AND tb_rekat.sd = '41' group BY tb_rekat.sd");
