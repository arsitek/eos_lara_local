    ...
    public function getAlokasiBackup(Request $req): JsonResponse
    {
        try {
            ...
            $validated = Validator::make($req->all(), [
                'idBackup' => 'required|integer|exists:tb_duplikasi_rkat,id',
            ], [
                'idBackup.required' => 'ID Backup harus diisi',
                'idBackup.integer' => 'ID Backup harus berupa angka',
                'idBackup.exists' => 'ID Backup tidak ditemukan',
            ]);
            if ($validated->fails()) {
                return response()->json(['success' => false, 'message' => 'Validasi gagal', 'errors' => $validated->errors()], 422);
            }
