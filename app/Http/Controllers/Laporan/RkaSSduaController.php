...
class RkaSSduaController extends Controller {
    public function index(){
        return view('content.laporan.KLASIFIKASI.SASARAN.DUA.dua');
    }
    public function pdfNonApbn(){
        $idunit = session()->get("unitkerja", "");
        $tahun = str_replace("_", " ", session()->get("tahun", "tahun_2025"));
        return view('content.laporan.KLASIFIKASI.SASARAN.DUA.pdf', compact('idunit', 'tahun'));
    }
    public function getNonApbn() {
        $tahun = session()->get("tahun", "tahun_2025");
        $kro = DB::connection('sirekat')->select("SELECT DISTINCT kd_ss, sasaran_program FROM iku_baru WHERE kd_ss = 'SKD' ");
        $ro = DB::connection('sirekat')->select("SELECT DISTINCT SUBSTRING(kode_ikk,5) AS kode_ikk, indikator_kinerja_kegiatan FROM iku_baru
                            WHERE kode_ikk LIKE '%IKU.2%' ORDER BY kode_ikk DESC");
        ...
        AND tb_rekat.sd = '41' AND tb_rekat.kd_kro = 'SKD' GROUP BY tb_rekat.unit_pelaksana, tb_rekat.kd_rk");
        ...
        $detail_kegiatan =  Rekat::SELECT("id","id_sub_judul", "kd_rk", "sub_judul", "unit_pelaksana", "rincian_komponen", "unit_kerja", "rpd")
                            ->where(['tahun' => $tahun, "sd" => '41'])->where("kd_kro", "=", "SKD")->get();
        ...
        OR tb_rabperalatan.id_jenis_belanja LIKE '%51%') AND tb_rekat.kd_kro = 'SKD' AND tb_rekat.tahun = '$tahun'
        ...
        OR tb_rabperalatan.id_jenis_belanja LIKE '%51%') AND tb_rekat.kd_kro = 'SKD'
        AND tb_rekat.sd = '41' ORDER BY tb_rabkegiatan.id_jenis_belanja desc,tb_rabgedung.id_jenis_belanja desc,
        tb_rabperalatan.id_jenis_belanja desc");
        ...
        $sum_coa = DB::connection('sirekat')->select("select tb_rekat.id_sub_judul,tb_rekat.unit_kerja,tb_rekat.kd_rk,
            ...
            OR tb_rabkegiatan.id_jenis_belanja LIKE '%51%'
            OR tb_rabperalatan.id_jenis_belanja LIKE '%51%') AND tb_rekat.kd_kro = 'SKD' AND tb_rekat.tahun = '$tahun'
            AND tb_rekat.sd = '41' GROUP BY tb_rabkegiatan.id_jenis_belanja, tb_rabkegiatan.id_jenis_belanja,tb_rabgedung.id_jenis_belanja,
       ...
       $sum_detail_kegiatan = DB::connection('sirekat')->select("select tb_rekat.id,
        ...
        $sum_unit_kerja = DB::connection('sirekat')->select("select tb_rekat.rincian_komponen,tb_rekat.unit_kerja,tb_rekat.kd_rk, tb_rekat.id_sub_judul,
            ...
            OR tb_rabkegiatan.id_jenis_belanja LIKE '%51%'
            OR tb_rabperalatan.id_jenis_belanja LIKE '%51%') AND tb_rekat.kd_kro = 'SKD' AND tb_rekat.tahun = '$tahun'
            AND tb_rekat.sd = '41' GROUP BY  tb_rekat.kd_rk, tb_rekat.unit_kerja, tb_rekat.unit_pelaksana");
               ...
               AND tb_rekat.kd_kro = 'SKD'
               AND tb_rekat.sd = '41' GROUP BY tb_rekat.rincian_komponen");
               ...
               AND tb_rekat.kd_kro = 'SKD' AND tb_rekat.tahun = '$tahun'
