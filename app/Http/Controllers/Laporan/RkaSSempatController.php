...
class RkaSSempatController extends Controller {
    public function index() {
        return view('content.laporan.KLASIFIKASI.SASARAN.EMPAT.empat');
    }
    public function pdfNonApbn() {
        $idunit = session()->get("unitkerja", "");
        $tahun = str_replace("_", " ", session()->get("tahun", "tahun_2025"));
        return view('content.laporan.KLASIFIKASI.SASARAN.EMPAT.pdf', compact('idunit', 'tahun'));
    }
    public function getNonApbn()
    {
        $tahun = session()->get("tahun", "tahun_2025");
        $kro = DB::connection('sirekat')->select("SELECT DISTINCT kd_ss, sasaran_program FROM iku_baru WHERE kd_ss = 'SKT' ");
        $ro = DB::connection('sirekat')->select("SELECT DISTINCT SUBSTRING(kode_ikk,5) AS kode_ikk, indikator_kinerja_kegiatan FROM iku_baru
            WHERE kode_ikk LIKE '%IKK.4%' ORDER BY kode_ikk DESC");
        $kp = DB::connection('sirekat')->select("SELECT DISTINCT kd_ikv, ikv FROM iku_baru ORDER BY RIGHT(kd_ikv, 5) DESC");
        $sk = IKK::SELECT(
            \DB::raw("SUBSTRING(kd_keg, 4, 5) AS kd_keg_compare"),
            \DB::raw("SUBSTRING(kd_keg, 10) AS kd_keg"),
            "rincian_kegiatan"
        )
            ->distinct()->orderBy("kd_keg", "DESC")->get();
        // ✅ Select data unit kerja dari tabel rekat
        $unit_kerja = DB::connection('sirekat')->select("SELECT DISTINCT 
                tb_rekat.id,tb_rekat.kd_rk, tb_rekat.sub_judul,tb_rekat.unit_pelaksana,
                tb_rekat.rincian_komponen,tb_unit.idunit, tb_unit.unitkerja,tb_rekat.kd_rk 
            FROM tb_rekat
            INNER JOIN tb_unit ON 
                tb_rekat.unit_kerja = tb_unit.idunit 
            WHERE tb_rekat.tahun = '$tahun'
                AND tb_rekat.sd = '41' 
                AND tb_rekat.kd_kro = 'SKT' 
            GROUP BY tb_rekat.unit_pelaksana, tb_rekat.kd_rk, tb_rekat.unit_kerja");
        // ✅ Select data detail kegiatan dari tabel rekat
        $detail_kegiatan =  Rekat::SELECT("id", "id_sub_judul", "kd_rk", "sub_judul", "unit_pelaksana", "rincian_komponen", 
        "unit_kerja", "rpd")
            ->where(['tahun' => $tahun, "sd" => '41'])
            ->where("kd_kro", "=", "SKT")
            ->get();
        // ✅ Select data coa -> join 3 tabel rab (keg, per, gdg)
        $rekat_coa = DB::connection('sirekat')->select("SELECT DISTINCT 
            tb_rekat.id, tb_rekat.unit_kerja,tb_rekat.sub_judul,tb_rekat.id_sub_judul,
            tb_rekat.rincian_komponen as rk_rekat, tb_rekat.kd_rk,
        ...
        WHERE (
            ...
            AND tb_rekat.sd = '41' 
            ORDER BY 
                tb_rabkegiatan.id_jenis_belanja desc,
                tb_rabgedung.id_jenis_belanja desc,
                tb_rabperalatan.id_jenis_belanja desc");

        $rekat_kk = DB::connection('sirekat')->select("SELECT 
            tb_unit.idunit,tb_unit.unitkerja,tb_rekat.id_sub_judul, tb_rekat.sub_judul, 
            tb_rekat.rincian_komponen AS rk_rekat, tb_rabkegiatan.id_kegiatan AS id_keg,
            ...
            tb_rabkegiatan.id_kegiatan, tb_rabkegiatan.id_jenis_belanja AS id_belanja_keg, 
            ...
            FORMAT( 
                (tb_rabgedung.DED_AWAL + tb_rabgedung.DED_REVIEW + tb_rabgedung.nilai_perencanaan
                + tb_rabgedung.nilai_struktur + tb_rabgedung.nilai_me + tb_rabgedung.nilai_landscape 
                + tb_rabgedung.nilai_pengawasan),0) AS biaya_satuan_gdg,
        ...
        AND tb_rekat.kd_kro = 'SKT' 
        AND tb_rekat.sd = '41' 
        ORDER BY tb_rabkegiatan.id_jenis_belanja desc,
        tb_rabgedung.id_jenis_belanja desc,
        tb_rabperalatan.id_jenis_belanja desc");

        $sum_coa = DB::connection('sirekat')->select("SELECT 
       ...
       WHERE (
        ...
        AND tb_rekat.tahun = '$tahun'
        AND tb_rekat.sd = '41' 
        GROUP BY tb_rabkegiatan.id_jenis_belanja, tb_rabkegiatan.id_jenis_belanja,
        tb_rabgedung.id_jenis_belanja, tb_rekat.id_sub_judul ");
        ...
        AND tb_rekat.tahun = '$tahun'
        AND tb_rekat.sd = '41' 
        GROUP BY tb_rekat.unit_kerja, tb_rekat.sub_judul");
        ...
        $sum_rincian_komponen = DB::connection('sirekat')->select("SELECT 
            ...
            WHERE tb_rekat.tahun = '$tahun' 
                AND tb_rekat.kd_kro = 'SKT' 
                AND tb_rekat.sd = '41' GROUP BY tb_rekat.kd_rk");
      
        $sum_ikv = DB::connection('sirekat')->select("SELECT 
            tb_rekat.rincian_kegiatan,tb_rekat.unit_kerja, tb_rekat.rincian_komponen,
        ...
        $sum_sd = DB::connection('sirekat')->select("SELECT 
            tb_rekat.sd, tb_rekat.unit_kerja, tb_rekat.sasaran_program,
