...
class TanggapanRabKegiatan extends Model
{
    protected $connection = 'sirekat';
    use HasFactory;
    protected $table = "tb_tanggapan_rabkegiatan";
    protected $fillable = ["id_rabkegiatan", "role", "tanggapan"];

    /**
     * Membentuk status dan isi tooltip berdasarkan verifikator yang benar-benar menolak.
     */
    public static function getRejectionInfo($rabKegiatan, $tanggapan): array
    {
        $verificationMap = [
            [
        ...
        foreach ($verificationMap as $verification) {
            if ($rabKegiatan->{$verification["column"]} !== "Tolak") {
                continue;
            }
