<script>
    $(document).ready( function () {
        ...
        $.ajax({
            type:'GET',
            url:" {{ route('rka.empat.getNonApbn') }} ",
			success:function(data){
                ...
                data.data.dk.forEach((item, index) => {
                    $(".unitkerja-non-apbn").each(function() {
                        ...
                        if ( item.unit_kerja == uk && kode_rk === item.kd_rk.substring(3) && item.unit_pelaksana == up) {
                            $(this).parent().after(`<tr>
                ...
                data.data.sum_coa.forEach((item, index) => {
                    $(".biaya").each(function(){
                        ...
                        let jb = ""
                        if ( $(this).attr("class").split(".")[3] != undefined ) {
                            jb = $(this).attr("class").split(".")[3].split("-")[0].replace(" ", "")
                        }
                ...
                data.data.sum_uk.forEach((item,index) => {
                    $('.unitkerja-non-apbn').each(function(){
                        ...
                        if( item.kd_rk.replace("[", "").substring(3) ==  kode_sk && rincian_td[2] == item.unit_kerja &&
                        item.unit_pelaksana == rincian_td[7]){
