<script type="text/javascript">
    $(document).ready(function () {
        $.ajax({
            type:'GET',
            url:" {{ route('rktReport.syncKroApbn') }} ",
			success:function(data){
                let bodyLaporan = $('.body-tbl')
                let kro = data.kro
                let ro = data.ro
                let kp = data.kp
                let sk = data.sk
                let uk = data.unit_kerja
                let rekat_coa = data.rekat_coa
                let rekat_kk = data.rekat_kk
                let coa = data.coa
                let sum_dk = data.sum_detail_kegiatan
                let sum_uk = data.sum_unit_kerja
                let sum_rk = data.sum_rincian_komponen
                let sum_ikv = data.sum_ikv
                let sum_ikk = data.sum_ikk
                let sum_ss = data.sum_ss
                // 52 = 42
                // 51 = 41  
            // tunggu beberapa detik untuk add row apbn 42 supaya tidak terjadi tabrakan script 
            setTimeout(() => {
                if (uk.length == 0){
                    return false
                }
                $('table tr:last').after('<tr><td>42</td><td class="apbn" colspan="8">APBN</td></tr>');
                
                kro.forEach((item, index) => {
                    $('.tabel-rekat tr:last').after(`<tr>
                        <td class="${item.kd_ss}-apbn">42.${item.kd_ss}</td>
                        <td colspan="10" class="ss-apbn">${item.sasaran_program}</td>
                    </tr>`)
                })
                let rowSKL = $('.SKL-apbn')
                let rowSKK = $('.SKK-apbn')
                let rowSKD = $('.SKD-apbn')
                let rowSKT = $('.SKT-apbn')
                // ✅ ro looping
                ro.forEach((item, index) => {
                    if(item.kode_ikk.slice(0,1) == "2"){
                        rowSKD.parent().after(`<tr>
                            <td class="ikk-apbn ${item.kode_ikk} SKD">42.SKD.${item.kode_ikk}</td>
                            <td class="ikk_text-apbn .${item.indikator_kinerja_kegiatan}" colspan="10">${item.indikator_kinerja_kegiatan}</td>
                        </tr>`)
                    }
                    if(item.kode_ikk.slice(0,1) == "3"){
                         rowSKK.parent().after(`<tr>
                            <td class="ikk-apbn ${item.kode_ikk} SKK">42.SKK.${item.kode_ikk}</td>
                            <td class="ikk_text-apbn .${item.indikator_kinerja_kegiatan}" colspan="10">${item.indikator_kinerja_kegiatan}</td>
                        </tr>`)
                    }
                    if(item.kode_ikk.slice(0,1) == "4"){
                         rowSKT.parent().after(`<tr>
                            <td class="ikk-apbn ${item.kode_ikk} SKT">42.SKT.${item.kode_ikk}</td>
                            <td class="ikk_text-apbn .${item.indikator_kinerja_kegiatan}" colspan="10">${item.indikator_kinerja_kegiatan}</td>
                        </tr>`)
                    }
                    if(item.kode_ikk.slice(0,1) == "1"){
                         rowSKL.parent().after(`<tr>
                            <td class="ikk-apbn ${item.kode_ikk} SKL">42.SKL.${item.kode_ikk}</td>
                            <td class="ikk_text-apbn .${item.indikator_kinerja_kegiatan}" colspan="10">${item.indikator_kinerja_kegiatan}</td>
                        </tr>`)
                    }
                })
                // ✅ kp looping
                kp.forEach((item, index) => {
                    let kd_ikv = item.kd_ikv.replace(/[A-Za-z]/g, "").substring(0,4)
                    $(".ikk-apbn").each(function() {
                        let ikk_td = $(this).attr('class').split(" ")
                        if(kd_ikv == "."+ikk_td[1].substring(0,3)){
                            $(this).parent().after(`<tr>
                                <td class="kd_ikv-apbn |${item.kd_ikv}|${ikk_td[2]}|${ikk_td[1]}|${item.kd_ikv.substring(7,8)+'.'}">42.${ikk_td[2]}.${ikk_td[1]}.${item.kd_ikv.substring(7,8)}</td>
                                <td colspan="10" class="ikv-apbn .${item.ikv}">${item.ikv}</td>
                            </tr>`)
                        }
                    })
                })
                // ✅ sub komponen looping
                sk.forEach((item, index) => {
                    $(".kd_ikv-apbn").each(function() {
                        let ikv_td = $(this).attr("class").split("|")
                        if(ikv_td[1].substring(3,8) == item.kd_keg_compare){
                            $(this).parent().after(`<tr>
                                <td class="kp-apbn ${item.kd_keg}">42.${ikv_td[2]}.${ikv_td[3]}.${ikv_td[4]}${item.kd_keg}</td>
                                <td colspan="8" class="rincian_kegiatan-apbn ${ikv_td[2]} ${ikv_td[3]} ${ikv_td[4]} ${item.kd_keg}">${item.rincian_kegiatan}</td>
                            </tr>`)
                        }
                    })
                })
                // ✅ unitkerja looping
                uk.forEach((item, index) => {
                    $(".rincian_kegiatan-apbn").each(function() {
                        let rincian_td = $(this).attr("class").split(" ")
                        if(item.rincian_komponen == $(this).text()){
                            $(this).parent().after(`<tr>
                                <td class="uk-apbn ${item.idunit}">${item.idunit}</td>
                                <td colspan="8" class="unitkerja-apbn =${item.rincian_komponen}=${item.idunit}=${rincian_td[1]}=${rincian_td[2]}=${rincian_td[3]}=${rincian_td[4]}">${item.unitkerja} | ${item.unit_pelaksana}</td>
                            </tr>`)
                        }
                    })
                })
                // ✅ Detail kegiatan looping
                data.detail_kegiatan.forEach((item, index) => {
                    $(".unitkerja-apbn").each(function() {
                        let unitkerja_td = $(this).attr('class').split("=")
                        let rk = unitkerja_td[1]
                        let uk = unitkerja_td[2]
                        if(uk == item.unit_kerja && item.rincian_komponen == rk){
                            $(this).parent().after(`<tr>
                                <td class="invisible">${item.id_sub_judul}</td>
                                <td colspan="8" class="dk-apbn =${item.sub_judul}=${item.unit_kerja}=${item.id_sub_judul}=${unitkerja_td[3]}=${unitkerja_td[4]}=${unitkerja_td[5]}=${item.id_sub_judul}=${unitkerja_td[6]}">${item.sub_judul}</td>
                                </tr>`)
                        }
                    })
                })
                // ✅ rekat coa(jenis belanja) looping
                rekat_coa.forEach((item,index) => {
                    $(".dk-apbn").each(function(){
                        let detail_td = $(this).attr("class").split("=")
                        let uk = detail_td[2]
                        let dk = detail_td[1]
                        if(uk == item.unit_kerja && dk == item.sub_judul){
                            $(this).parent().after(`<tr>
                                <td>42.${detail_td[4]}.${detail_td[5]}.${detail_td[6]}.${detail_td[8]}.${uk}.${
                                    item.id_belanja_keg ? item.id_belanja_keg : (item.id_belanja_per) ? item.id_belanja_per : item.id_belanja_gdg
                                }</td>
                                <td colspan="10" class="jb-apbn |${item.rk_rekat}|${
                                    item.id_belanja_keg ? item.id_belanja_keg : (item.id_belanja_per) ? item.id_belanja_per : item.id_belanja_gdg
                                }|${item.unit_kerja}">${
                                    item.belanja_keg ? item.belanja_keg : (item.belanja_per) ? item.belanja_per : item.belanja_gdg
                                }</td>
                                <td style="text-align: right;" class="biaya-apbn .${item.id_sub_judul}.${item.unit_kerja}.${
                                    item.id_belanja_keg ? item.id_belanja_keg : (item.id_belanja_per) ? item.id_belanja_per : item.id_belanja_gdg
                                }"></td><td></td></tr>`)
                        }
                    })
                })
                // ✅ rekat kk(kebutuhan kegiatan) looping
                rekat_kk.forEach((item, index) => {
                    let bintang = []
                    let html_status = ''
                    if(item.id_belanja_keg != null){
                        bintang = []
                        const verifikasi_array = [item.verifikasi_tim_keg,item.verifikasi_pimpinan_keg,item.verifikasi_univ_keg];
                        let total_ver = verifikasi_array.filter(word => word === "SILAHKAN PILIH" || word === null || word === "Tolak");
                        for (let i = 0; i < total_ver.length; i++) {
                            bintang.push("*")
                        }
                    }else if(item.id_belanja_per != null){
                        bintang = []
                        const verifikasi_array = [item.verifikasi_tim_per,item.verifikasi_pimpinan_per,item.verifikasi_univ_per];
                        let total_ver = verifikasi_array.filter(word => word === "SILAHKAN PILIH" || word === null);
                        for (let i = 0; i < total_ver.length; i++) {
                            bintang.push("*")
                        }
                    }else if(item.id_belanja_gdg != null){
                        bintang = null
                        const verifikasi_array = [item.verifikasi_tim_gdg,item.verifikasi_pimpinan_gdg,item.verifikasi_univ_gdg];
                        let total_ver = verifikasi_array.filter(word => word === "SILAHKAN PILIH" || word === null);
                        for (let i = 0; i < total_ver.length; i++) {
                            bintang.push("*")
                        }
                    }
                    if ( bintang.length !== 0 ) {
                        html_status = "<span class='px-1 py-1 text-white ml-1' style='font-size:9px;background-color:red'>Belum diverifikasi</span>"
                    }
                    $(".jb-apbn").each(function(){
                        let jb_td = $(this).attr("class").split("|")
                        let rincian_komponen = jb_td[1]
                        let id_belanja = jb_td[2]
                        let uk = jb_td[3]
                        if(item.rk_rekat == rincian_komponen && item.idunit == uk){
                            if(item.id_belanja_keg == id_belanja || item.id_belanja_per == id_belanja || item.id_belanja_gdg == id_belanja){
                                $(this).parent().after(`<tr>
                                    <td style="text-align:right">-</td>
                                    <td class="kk .${item.detail_kegiatan}.${item.idunit}.${
                                    item.id_belanja_keg ? item.id_belanja_keg : (item.id_belanja_per) ? item.id_belanja_per : item.id_belanja_gdg}">${item.kebutuhan_keg ? item.kebutuhan_keg : (item.kebutuhan_per) ? item.kebutuhan_per : item.kebutuhan_gdg} ${html_status}</td>
                                    <td id="kuantitas">${item.kuantitas ? item.kuantitas : (item.qt) ? item.qt : 1}</td>
                                    <td>${item.satuan_kuantitas ? item.satuan_kuantitas : (item.satuan_qt_per) ? item.satuan_qt_per : 'Orang'}</td>
                                    <td id="durasi">${item.durasi ? item.durasi : 1}</td>
                                    <td>${item.satuan_durasi ? item.satuan_durasi : 'Pkt'}</td>
                                    <td>${item.kegiatan ? item.kegiatan : 1}</td>
                                    <td>${item.satuan_kegiatan ? item.satuan_kegiatan : 'Pkt'}</td>
                                    <td style="text-align: right;">${
                                    item.biaya_satuan ? item.biaya_satuan : (item.biaya_satuan_per) ? item.biaya_satuan_per : item.biaya_satuan_gdg}</td>
                                    <td style="text-align: right;" class="biaya_kk">${
                                    item.total_per ? item.total_per : (item.total_gdg) ? item.total_gdg : item.total_keg
                                    }</td><td></td><td>realisasi</td><td></td></tr>`)
                            }
                        }
                    })
                })
                // ✅ the sum looping is begin in this line :
                // 1. jenis belanja sum
                coa.forEach((item, index) => {
                    $(".biaya-apbn").each(function(){
                        let dk = $(this).attr("class").split(".")[1]
                        let uk = $(this).attr("class").split(".")[2]
                        let jb = $(this).attr("class").split(".")[3]
                        if(dk == item.id_sub_judul && uk == item.unit_kerja){
                            if(jb == item.id_belanja_keg || jb == item.id_belanja_per || jb == item.id_belanja_gdg){
                            if(item.total_coa_keg != null){
                                return $(this).text(item.total_coa_keg)}
                            else if(item.total_coa_per != null){
                                return $(this).text(item.total_coa_per)}
                            else if(item.total_coa_gdg != null){
                                return $(this).text(item.total_coa_gdg)}
                            }
                        }
                    })
                })
                //2. Detail kegiatan sum
                sum_dk.forEach((item, index) => {
                    $(".dk-apbn").each(function(){
                        let dk = $(this).attr("class").split("=")[1]
                        let uk = $(this).attr("class").split("=")[2]
                        let id_dk = $(this).attr("class").split("=")[3]
                        if(uk == item.unit_kerja && id_dk == item.id_sub_judul){
                            let dk_td = `
                            <td style="text-align: right;" class="biaya_dk">`
                            if(item.total_dk_per != null){
                                dk_td += `${item.total_dk_per}</td><td>${item.rpd}</td><td></td><td></td>`
                                return $(this).after(dk_td)}
                            else if(item.total_dk_gdg != null){
                                dk_td += `${item.total_dk_gdg}</td><td>${item.rpd}</td><td></td><td></td>`
                                return $(this).after(dk_td)}
                            else if(item.total_dk_keg != null){
                                dk_td += `${item.total_dk_keg}</td><td>${item.rpd}</td><td></td><td></td>`
                                return $(this).after(dk_td)}
                            $(this).after(dk_td)
                        }
                    })
                })
                 //3. Unit kerja sum
                sum_uk.forEach((item,index) => {
                    $('.unitkerja-apbn').each(function(){
                        let rincian_komponen = $(this).attr("class").split("=")[1]
                        let unitkerja = $(this).attr("class").split("=")[2]
                        if(unitkerja == item.unit_kerja && rincian_komponen == item.rincian_komponen){
                            let uk_td = `
                            <td class="biaya_uk" style="text-align: right;">`
                            if(item.total_uk_per != null){
                                uk_td += `${item.total_uk_per}</td><td></td><td></td><td></td>`
                                return $(this).after(uk_td)}
                            else if(item.total_uk_gdg != null){
                                uk_td += `${item.total_uk_gdg}</td><td></td><td></td><td></td>`
                                return $(this).after(uk_td)}
                            else if(item.total_uk_keg != null){
                                uk_td += `${item.total_uk_keg}</td><td></td><td></td><td></td>`
                                return $(this).after(uk_td)}
                            $(this).after(uk_td);
                        }
                    })
                })
                //4. Rincian komponen sum
                sum_rk.forEach((item,index) => {
                    $('.rincian_kegiatan-apbn').each(function(){
                        let rincian_komponen = $(this).text()
                        if(rincian_komponen == item.rincian_komponen){
                            let rk_td = `
                            <td style="text-align: right;">${item.TOTAL_RK}</td><td></td><td></td><td></td>`
                            return $(this).after(rk_td)
                        }
                    })
                })
                //5. Ikv sum 
                sum_ikv.forEach((item, index) => {
                    $('.ikv-apbn').each(function(){
                        let ikv = $(this).attr("class").split(".")[1]
                        if($(this).text() === item.rincian_kegiatan.split("] ")[1]){
                            // console.log($(this).text())
                            let ikv_td = `
                            <td style="text-align: right;">${item.TOTAL_IKV}</td><td></td>`
                            return $(this).after(ikv_td)
                        }
                    })
                })
                // 6. Ikk sum
                sum_ikk.forEach((item, index) => {
                    $('.ikk_text-apbn').each(function(){
                        let ikk_text = $(this).attr("class").split("] ")[1]
                        if(ikk_text == item.indikator_kinerja_kegiatan.split("] ")[1]){
                            let ikk_td = `
                            <td style="text-align: right;">${item.TOTAL_IKK}</td><td></td>`
                            return $(this).after(ikk_td)
                        }
                    })
                })
                //7. Sasaran sum
                sum_ss.forEach((item, index) => {
                    $('.ss-apbn').each(function(){
                        let ss = $(this).text()
                        if(ss == item.sasaran_program){
                            let ss_td = `
                            <td style="text-align: right;" class="biaya_ss">${item.TOTAL_SS}</td><td></td>`
                            $(this).after(ss_td)
                        }
                    })
                })
                data.sum_sd.forEach(item => {
                    $(".apbn").after(`<td style="text-align:right">${item.TOTAL_SD}<td><td></td><td></td>`)
                })
                clearUnwantedRow();

                }, 1000);
			},
            error: function (request, status, error) {
		        tata.error('Galat', 'Gagal menghapus data', {
    			duration : 3000,
				animate  : slide
			})
		}}) // End ajax 
        function clearUnwantedRow(){
            $(".ss-apbn").each(function(){
                    if(!$(this).closest('td').next('td')[0]){
                        $(this).closest('tr').remove()
                    }
                })
                $(".dk-apbn").each(function(){
                    if(!$(this).closest('td').next('td')[0]){
                        $(this).closest('tr').remove()
                    }
                })
                $(".ikk_text-apbn").each(function(){
                    if(!$(this).closest('td').next('td')[0]){
                        $(this).closest('tr').remove()
                    }
                })
                $(".ikv-apbn").each(function(){
                    if(!$(this).closest('td').next('td')[0]){
                        $(this).closest('tr').remove()
                    }
                })
                $(".rincian_kegiatan-apbn").each(function(){
                    if(!$(this).closest('td').next('td')[0]){
                        $(this).closest('tr').remove()
                    }
                })
                $(".unitkerja-apbn").each(function(){
                    if(!$(this).closest('td').next('td')[0]){
                        $(this).closest('tr').remove()
                    }
                })
        }
    })
</script>