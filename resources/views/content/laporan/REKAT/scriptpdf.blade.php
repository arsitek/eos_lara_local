...
<script type="text/javascript">
    $(document).ready(function () {
			...
			success:function(data){
                ...
                ro.forEach((item, index) => {   
                    if(item.kode_ikk.slice(0,1) == "2"){
                         rowSKD.parent().after(`<tr>
                            ...
                            <td colspan="8" class="ikk_text .${item.indikator_kinerja_kegiatan} =${item.kode_ikk}">${item.indikator_kinerja_kegiatan}</td>
                        </tr>`)
                    }
                    ...
                    $(".ikk").each(function() {
                        ...
                        if(kd_ikv == "."+$(this).attr('class').substring(4,7)){
                            $(this).parent().after(`<tr>
                                ...
                                <td colspan="8" class="ikv .${item.ikv} 41.${ikk_td[2]}.${ikk_td[1]}.${item.kd_ikv.substring(7,8)+'.'}">${item.ikv}</td>
                            </tr>`)
                        }
                    })
                })
                    ...
                    $(".kp").each(function() {
                        ...
                        if(kp[1].substring(3) == item.kd_keg_compare){
                            $(this).parent().after(`<tr>
                                ...
                                <td colspan="8" class="rincian_kegiatan ${kp[2]} ${kp[3]} ${kp[1].substring(7,8)} ${item.kd_keg}">${item.rincian_kegiatan}</td>
                            </tr>`)
                        }
                    })
                })
                ...
                uk.forEach((item, index) => {
                    $(".rincian_kegiatan").each(function() {
                        ...
                        if(item.rincian_komponen == $(this).text()){
                            $(this).parent().after(`<tr>
                                <td class="uk ${item.idunit}"style="text-align:left">${item.idunit}</td>
                                <td colspan="8" class="unitkerja =${item.rincian_komponen}=${item.idunit}=${rincian_td[1]}=${rincian_td[2]}=${rincian_td[3]}=${rincian_td[4]}=${item.unit_pelaksana}=${item.id_sub_judul}">${item.unitkerja} | ${item.unit_pelaksana}</td>
                            </tr>`)
                        }
                    })
                })
                ...
                data.detail_kegiatan.forEach((item, index) => {
                    $(".unitkerja").each(function() {
                        ...
                        let up = unitkerja_td[7]
                        let sj = unitkerja_td[8]
                        if( uk == item.unit_kerja && item.rincian_komponen == rk  ){
                        // if(uk == item.unit_kerja && item.rincian_komponen == rk && item.unit_pelaksana == up){
                            $(this).parent().after(`<tr>
                                <td style="font-weight:bold">${item.id}</td>
                                <td style="font-weight:bold" colspan="8" class="dk =${item.sub_judul}=${item.unit_kerja}=${item.id_sub_judul}=${unitkerja_td[3]}=${unitkerja_td[4]}=${unitkerja_td[5]}=${item.id_sub_judul}=${unitkerja_td[6]}">${item.sub_judul}</td>
                                </tr>`)
                        }     
                    })
                })
                ...
                rekat_coa.forEach((item,index) => {
                    $(".dk").each(function(){
                        ...
                        if( kode == item.kd_rk.substring(4) && item.id_sub_judul == sj){
                ...
                coa.forEach((item, index) => {
                    $(".biaya").each(function(){
                        ...
                        if ( jb ) { 
                            if(!jb.length < 6 || !jb == undefined) {
                                jb = jb.split("-")[0].replace(" ", "")
                            }
                        }
                        let sj = $(this).attr("class").split(".")[1]
                        if ( biaya_td[1] != undefined) {
                            if ( item.kd_rk.substring(4) == biaya_td[1].substring(7) && item.id_sub_judul == sj) {
                ...
                sum_dk.forEach((item, index) => {
                    $(".dk").each(function(){
                        ...
                        if(uk == item.unit_kerja && id_dk == item.id_sub_judul && kode_sk == item.kd_rk.substring( 4 )){
                ...
                sum_uk.forEach((item,index) => {
                    $('.unitkerja').each(function(){
                        ...
                        if( item.kd_rk.substring( 4 ) ==  kode_sk && item.id_sub_judul == rincian_td[8] && rincian_td[2] == item.unit_kerja){
                ...
                sum_rk.forEach((item,index) => {
                    $('.rincian_kegiatan').each(function(){
                        ...
                        // if( item.kd_rk.substring( 4 ) ==  kode_sk ){
                        //     let rk_td = `<td style="text-align: right;">${item.TOTAL_RK}</td><td></td><td></td>`
                        //     return $(this).after(rk_td)
                        // }
                    })
                })
                ...
                sum_ss.forEach((item, index) => {
                    $('.ss').each(function(){
                        ...
                        if(ss == item.sasaran_program){
                            let ss_td = `
                            <td style="text-align: right;">${item.TOTAL_SS}</td><td></td><td></td><td></td>`
                ...
                data.sum_sd.forEach(item => {
                    $(".non-apbn").after(`<td style="text-align:right">${item.TOTAL_SD}<td><td></td>`)
                })
                clearUnwantedRow()
        ...
        //     console.log(tabel)
        //     // console.log(html2pdf().from(tabel).save())
        // })
    })
</script>   
