          ...
          return new Promise((resolve, reject) => {
            try {
              ...
              alokasi.forEach(item => {
                ...
                const jenisSumberDana = kodeSd.startsWith("41") ? "Non APBN" : kodeSd.startsWith("42") ?
                  "APBN" : "Lainnya"
                const jenisMap = createOrUpdateMap(baseData, jenisSumberDana, () => ({
                  pagu: 0,
                  idunit,
                  sub: new Map()
                }))
                const unitMap = createOrUpdateMap(jenisMap.sub, idunit, () => ({
                  pagu: 0,
                  idunit,
                  namaUnit,
                  sub: new Map()
                }))
                const sdMap = createOrUpdateMap(unitMap.sub, kodeSd, () => ({
                  pagu: 0,
                  kodeSd,
                  sumberdana
                }));
              ...
              realisasi.forEach(({
                ...
                if (!sd) return;
                const jenisSumberDana = sd.startsWith("41") ? "Non APBN" : sd.startsWith("42") ?
                  "APBN" : "Lainnya";
          ...
          return new Promise((resolve, reject) => {
            try {
              ...
              data.alokasi.forEach(item => {
                ...
                const sdMap = createOrUpdateMap(unitMap.sub, sd, () => ({
                  pagu: 0,
                  sd,
                  realisasi: 0,
                  sumberdana
              ...
              data.realisasi.forEach(({
                unit_kerja_rkt: idunit,
                jumlah_amprah,
                kd_sumberdana: kodeSd,
                sumberdana
              }) => {
        ...
        displayDataToTable: (data) => {
          return new Promise((resolve, reject) => {
            try {
              const {
                table
              } = window.dayaserap.elements
              const {
                isPdf
              } = window.dayaserap.variables
