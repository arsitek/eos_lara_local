...
$(document).ready(function() {
    ...
    $( document ).on("click", ".btn-save", function() {
        ...
        $.ajax({
            type: "POST", url: "{{ route('vRabGdg.store') }}",
            data: { "_token": "{{ csrf_token() }}",
                id_rab, rpd, id_coa, coa, kk, lang, platform, userAgent, screenSize, kode_aset, aset, jenis
            },
            success: ( res ) => {
                // console.log( res )
                const data = res.data
    ...
    $( document ).on("click", ".btn-filter-unitkerja", function(){
        ...
        window.open(`/verifikasi/gedung/${idunit.val()}?kd_sumberdana=${kodeSd.val()}`, "_self")
    })
