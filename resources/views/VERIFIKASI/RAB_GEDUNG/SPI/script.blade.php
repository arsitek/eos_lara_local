<script>
    $(document).ready(function() {
        // Jalur SPI dipisahkan agar filter dan simpan tidak memakai endpoint umum Gedung.
        ...
        const datatable = $('.tabel-vergdg').DataTable({
            "ordering": false,
            "autoWidth": false,
        ...
        $(document).on("click", ".btn-filter-unitkerja", function(){
            ...
            window.open(`/verifikasi/gedung-spi/${idunit.val()}?kd_sumberdana=${kodeSd.val()}`, "_self")
        })
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        error: function ( error ) {
                            const msg = error.responseJSON?.message || "Gagal menyetujui gedung"
        ...
        $(document).on("click", ".switchVerifikasi", (e) => {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('vRabGdgSpi.storeVerif') }}",
