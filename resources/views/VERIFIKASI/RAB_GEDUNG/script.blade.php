<script>
    $( document ).ready( function() {
        ...
        $( "select[name='jumlah_data']" ).on("change", function (e) {
            const jumlah_data   = $(this).val()
            const kd_sumberdana = document.querySelector('select[name=sumberdana]').value
            const status        = document.querySelector('select[name=filter_status]').value
            fetchEntries( jumlah_data, kd_sumberdana, status )
        })
        $( "#btn-cari-id-rekat" ).on("click", function(){
            $(".modal-id-rekat").modal("show")
        })
        $( "#close-modal" ).on("click", function(){
            $(".modal-id-rekat").modal("hide")
        })
        $( "#btn-submit-id-rekat" ).on("click", function(){
            const url      = new URL(window.location.href)
            const id_rekat = $("#id_rekat").val()
            url.searchParams.set("id_rekat", id_rekat)
        ...
        $(document).on("click", "button#btn_triggerTanggapan", function(){
            ...
            $.ajax({
                type: 'GET',
                url: "{{ route('vRabGdg.get.tanggapan') }}",
                data: { id },
                success: ( res ) => {
                    // clear tanggapan
                ...
                error: ( error ) => {
                    const msg = error.responseJSON.message || "Gagal mendapatkan tanggapan"
        ...
        $( document ).on( "click", ".btn_simpanTanggapan", function(){
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('vRabGdg.storeTanggapan') }}",
