<script>
    $(document).ready(function() {
        // Jalur SPI dipisahkan agar filter dan simpan tidak memakai endpoint umum Langganan.
        ...
        $(document).on("click", ".btn-filter-unitkerja", function(){
            ...
            window.open(`/verifikasi/langganan-spi/${idunit.val()}?kd_sumberdana=${kodeSd.val()}`, '_self')
        })
        ...
        $("#btn-setujui-semua").off("click").on("click", function(){
            Swal.fire({
                title: 'Apakah anda yakin untuk menyetujui semua langganan?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('vRabLanggananSpi.store') }}",
                        ...
                        error: function (error) {
                            const msg = error.responseJSON?.message || "Gagal menyetujui langganan"
        ...
        $(document).on("click", ".switchVerifikasi", (e) => {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('vRabLanggananSpi.storeVerif') }}",
